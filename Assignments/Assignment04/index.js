...
Object.keys(state_info).forEach(function(state) {
    var url = "https://api.forecast.io/forecast/a0e9b944cb4e28bf6bcd9155d5fd08c4/" + state_info[state]['lat'] + "," + state_info[state]['lng'];
    var x;
    $.ajax({
        url: url,
        dataType: "jsonp"
        ...
        function(data) {
            ...
            if (x <= 10.0)
                $('#' + state).css('fill', 'blue')
            else if (x >= 11.0 && x <= 30.0)
                $('#' + state).css('fill', 'cyan')
            else if (x >= 31.0 && x <= 50.0)
                $('#' + state).css('fill', 'green')
            else if (x >= 51.0 && x <= 80.0)
